...
# Holidays are defined as MM-DD only; year is auto-set to match the project start year.
# Both the start year and the following year are included to handle year-spanning schedules.
_HOLIDAY_MMDD = [
...
# HOLIDAYS list is populated after the start date is resolved (see below).
HOLIDAYS = []



# ''' obtain holidays in Japan '''

# year = int( CHART_START_DATE.split('-')[0] )  # obtain year info from CHART_START_DATE

# HOLIDAYS = [ dn[0].strftime("%Y-%m-%d") for dn in jpholiday.year_holidays( year )]

# ''' add holidays here if it is necessary
# HOLIDAYS.append('2024-12-25')
#'''
...
OUT_PNG = "./gantt_chart.png"
OUT_SVG = "./gantt_chart.svg"
...
def _parse_segments(seg_str: str):
    ...
    for token in seg_str.split(","):
        ...
        if not token:
            continue
        s, d = token.split("+")
        result.append((int(s), int(d)))
    return result


def load_schedule(filepath: str) -> tuple:
    """Pass 1 — read the file, extract startdate header and raw segment data.

    Returns (raw_rows, startdate_str_or_None) where raw_rows is a list of
    (name, plan_segs, actual_segs) tuples with integer offsets/durations.
    ...
    for raw in lines:
        ...
        if len(parts) < 2:
            continue
        name        = parts[0]
        plan_segs   = _parse_segments(parts[1])
        actual_segs = _parse_segments(parts[2]) if len(parts) >= 3 else []
...
# ── Pass 1: read file, extract startdate and raw rows ────────────────────────
print(f"Loading schedule from: {SCHEDULE_FILE}")
_raw_rows, _file_date = load_schedule(SCHEDULE_FILE)
...
# ── Generate HOLIDAYS for the resolved year ───────────────────────────────────
_chart_year = _proj_start_dt.year
for _mmdd, _label in _HOLIDAY_MMDD:
    for _yr in (_chart_year, _chart_year + 1):
        HOLIDAYS.append(f"{_yr}-{_mmdd}")
print(f"Holidays generated for {_chart_year}/{_chart_year + 1}  ({len(HOLIDAYS)} entries)")
...
def draw_bar(ax, y_centre, offset, start_dt, end_dt, working_days, color, alpha, zorder,
    ...
    # Duration label centred over the full bar
    label = f"{working_days}d"
    if cal_width >= 4:
        x_txt, ha, color_txt, x_off = left_num + cal_width / 2, "center", "white", 0
    else:
        x_txt, ha, color_txt, x_off = left_num + cal_width, "left", color, 2

    ax.annotate(
        label,
        xy=(x_txt, y_bar), xytext=(x_off, 0),
        textcoords="offset points",
        ha=ha, va="center",
        color=color_txt, zorder=zorder + 1,
        **_fprop(fontsize=6.5, fontweight="bold"),
    )
...
def main():
    ...
    # ── Title & subtitle ──────────────────────────────────────────────────────
    fig.suptitle("Project Schedule — Planned vs Actual", x=0.5, y=1.01,
                 color="#222222", **_fprop(fontsize=13, fontweight="bold"))
    ax.set_title(f"Working-day mode {WORK_MODE}: {MODE_LABELS[WORK_MODE]}",
                 color="#666666", pad=28, **_fprop(fontsize=8.5))
